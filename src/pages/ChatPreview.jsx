...
const ChatPreview = () => {
  const [messages, setMessages] = useState([
    ...
    { text: "clicking on the ArrowUpCircle should send user to a new page that should look exactly like this (see image):", sender: 'gpt' },
    { text: "Here's an example of how to add the icon:", sender: 'gpt' },
    { text: "import { ArrowUpCircle } from 'lucide-react';\n\n// Inside your component\n{isTyping && (\n  <ArrowUpCircle\n    className=\"w-5 h-5 text-gray-400 cursor-pointer\"\n    onClick={() => navigate('/chat-preview')}\n  />\n)}", isCode: true, sender: 'gpt' },
    { text: "And here's how you might style the new page:", sender: 'gpt' },
    { text: "<div className=\"h-screen bg-black text-white flex\">\n  <div className=\"w-1/2 p-4 overflow-y-auto\">\n    {/* Chat messages */}\n  </div>\n  <div className=\"w-1/2 p-4 border-l border-gray-700\">\n    {/* Preview iframe */}\n    <iframe src=\"/\" className=\"w-full h-full border-0\" />\n  </div>\n</div>", isCode: true, sender: 'gpt' },
  ]);
  ...
  const handleSendMessage = () => {
    if (inputMessage.trim()) {
      const isCode = inputMessage.includes('```');
      const newMessage = {
        text: isCode ? inputMessage.replace(/```(jsx)?|```/g, '').trim() : inputMessage,
        sender: 'user',
        isCode
      };
